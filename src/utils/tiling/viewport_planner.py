...
def plan_viewport_tiles(
    ...
    Returns:
      - ViewportPlan with core and requested tile ranges (inclusive indices).
    """
    ...
    clamped_viewport_level0 = _clamp_rect_to_bounds(rect=input_viewport_level0, bounds=source_bounds_level0)
...
def _clamp_rect_to_bounds(*, rect: Rect, bounds: Rect) -> Rect:
    ...
    clamped_width = max(0, min(rect.width, max_width_from_x))
    clamped_height = max(0, min(rect.height, max_height_from_y))
...
def _rect_pixels_to_tile_ranges(
    ...
    """
    Convert a pixel rectangle on a level grid into core and requested tile index ranges.
